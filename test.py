...
def recursive_convert_diffuse():
    ...
    print("⚙️  操作: 转换为 BC1 (DXT1) + 生成 Mipmaps\n")
        ...
        for filename in files:
            if filename.lower().endswith(".dds"):
                ...
                if "_diffuse" not in filename.lower():
                    skipped_count += 1
                    # print(f"  [跳过] 非Diffuse文件: {filename}") # 如果想看跳过了什么，取消注释
                    continue
                # --------------------
    ...
    print("-" * 30)
    print(f"统计结果:")
    print(f"✅ 成功处理 (Diffuse): {processed_count} 个文件")
    print(f"⏭️ 跳过文件 (Others) : {skipped_count} 个文件")

if __name__ == "__main__":
    recursive_convert_diffuse()
    input("\n按回车键退出...")