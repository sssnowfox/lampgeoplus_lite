...
# parse countries inside default block
countries = []  # list of tuples (country_code, start_idx, end_idx)
pos = body.find('{') + 1
L = len(body)
while pos < L:
    if body[pos].isspace():
        pos += 1
        continue
    m = re.match(r"([A-Z][A-Z0-9]*)\s*=\s*\{", body[pos:])
    if not m:
        pos += 1
        continue
    country = m.group(1)
    country_start = pos + m.start()
    brace_start = pos + m.end() - 1
    brace_end = find_matching(body, brace_start)
    if brace_end == -1:
        break
    country_end = brace_end + 1
    countries.append((country, country_start, country_end))
    pos = country_end

changed = 0
new_body = body

# iterate countries in reverse
for country, c_sidx, c_eidx in reversed(countries):
    country_text = new_body[c_sidx:c_eidx]
    
    # parse tank types inside each country
    tanks = []
    pos = country_text.find('{') + 1
    L_tank = len(country_text)
    while pos < L_tank:
        if country_text[pos].isspace():
            pos += 1
            continue
        m = re.match(r"([a-z_0-9]+)\s*=\s*\{", country_text[pos:])
        if not m:
            pos += 1
            continue
        tank_name = m.group(1)
        tank_start = pos + m.start()
        brace_start = pos + m.end() - 1
        brace_end = find_matching(country_text, brace_start)
        if brace_end == -1:
            break
        tank_end = brace_end + 1
        tanks.append((tank_name, tank_start, tank_end))
        pos = tank_end
    
    changed_in_country = 0
    new_country_text = country_text
    
    # iterate tanks in reverse
    for tank_name, t_sidx, t_eidx in reversed(tanks):
        tank_text = new_country_text[t_sidx:t_eidx]
        
        # find pool = { occurrences
        pool_positions = [m.start() for m in re.finditer(r"pool\s*=\s*\{", tank_text)]
        if len(pool_positions) != 2:
            continue
        
        # get actual pool blocks
        p1_start = pool_positions[0]
        p1_brace = tank_text.find('{', p1_start)
        p1_end = find_matching(tank_text, p1_brace)
        
        p2_start = pool_positions[1]
        p2_brace = tank_text.find('{', p2_start)
        p2_end = find_matching(tank_text, p2_brace)
        
        if p1_end == -1 or p2_end == -1:
            continue
        
        p1 = tank_text[p1_start:p1_end+1]
        p2 = tank_text[p2_start:p2_end+1]
        ...
        if re.search(r"models\s*=\s*\{", p1):
            ...
            if len(non_alt) == 1:
                ...
                models_text += indent + '}'
            # insert after icons block if exists
        ...
        if alt:
            ...
            # remove models block entirely
            p2_new = re.sub(r"\n\s*models\s*=\s*\{[\s\S]*?\}", '', p2, count=1)
        
        # replace both pools in tank_text
        tank_new = tank_text[:p1_start] + p1_new + tank_text[p1_end+1:p2_start] + p2_new + tank_text[p2_end+1:]
        new_country_text = new_country_text[:t_sidx] + tank_new + new_country_text[t_eidx:]
        changed_in_country += 1
    
    if changed_in_country > 0:
        new_body = new_body[:c_sidx] + new_country_text + new_body[c_eidx:]
        changed += changed_in_country
        print(f"{country}: Modified {changed_in_country} tank types")

if changed:
    new_default = new_body
    new_text = text[:start] + new_default + text[end:]
    src.write_text(new_text, encoding='utf-8')
    print(f"\n✅ Total: Modified {changed} tank type models. Backup at {backup}")
else:
